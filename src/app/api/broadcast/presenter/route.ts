...
export const GET = withErrorHandler(async (req: Request) => {
  ...
  const matchId = searchParams.get('matchId') ?? ''; const userId = resolveTargetUserId(ctx, searchParams.get('userId')); return NextResponse.json({ presenter: getPresenterState(matchId, userId) });
});
