/** GET /api/learning/plans/:id/next-lesson — Get the next pending lesson in the plan */
import { NextResponse } from "next/server";
import { withErrorHandler, notFound } from "@/lib/errors";
import { getAuthContext } from "@/features/auth";
import { getPlan, recommendNextLesson } from "@/features/learning-planner";

export const GET = withErrorHandler<{ id: string }>(async (_req, ctx) => {
  const authCtx = await getAuthContext();
  if (!authCtx.userId) {
    return NextResponse.json(
      { error: { code: "UNAUTHORIZED", message: "Authentication required" } },
      { status: 401 },
    );
  }
  const { id } = await ctx.params;
  const existing = await getPlan(id);
  if (!existing || existing.userId !== authCtx.userId) {
    throw notFound("Plan not found");
  }
  const next = await recommendNextLesson({
    userId: authCtx.userId,
    planId: id,
    locale: authCtx.locale ?? "en",
  });
  if (!next) {
    return NextResponse.json(
      { message: "No pending lessons in this plan" },
      { status: 404 },
    );
  }
  return NextResponse.json(next);
});
