/** System 23 — Event Bus Bridge. Passive consumer + producer. */
import { getLogger } from "@/lib/logger";
import { subscribe, emitEvent, type GameEvent, type GameEventType } from "@/features/game-engine";
import type { DataEventType } from "./types";

const log = getLogger("data.bridge");
const processedEventIds = new Set<string>();
const publishedEvents: Array<{ type: DataEventType; actorId: string | null; payload: Record<string, unknown>; timestamp: string }> = [];
...
export function subscribeDataPlatform(): void {
...
export function unsubscribeDataPlatform(): void { for (const u of unsubscribers) { try { u(); } catch {} } unsubscribers.length = 0; isSubscribed = false; processedEventIds.clear(); }
export function isDataPlatformSubscribed(): boolean { return isSubscribed; }
...
export function publishDataEvent(type: DataEventType, actorId: string | null, payload: Record<string, unknown>): void {
  publishedEvents.push({ type, actorId, payload, timestamp: new Date().toISOString() });
  emitEvent("data-platform", "StateTransition" as GameEventType, actorId, { dataEventType: type, ...payload });
  log.info("bridge.publish", { type, actorId });
}

export function _resetBridgeForTesting(): void { unsubscribeDataPlatform(); processedEventIds.clear(); publishedEvents.length = 0; }
