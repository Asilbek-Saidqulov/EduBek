...
const log = getLogger("integration.bridge");
const processedEventIds = new Set<string>();
const publishedEvents: Array<{ type: IntegrationEventType; actorId: string | null; payload: Record<string, unknown>; timestamp: string }> = [];
...
export function subscribeIntegration(): void {
...
export function unsubscribeIntegration(): void { for (const u of unsubscribers) { try { u(); } catch {} } unsubscribers.length = 0; isSubscribed = false; processedEventIds.clear(); }
export function isIntegrationSubscribed(): boolean { return isSubscribed; }
...
export function publishIntegrationEvent(type: IntegrationEventType, actorId: string | null, payload: Record<string, unknown>): void {
  publishedEvents.push({ type, actorId, payload, timestamp: new Date().toISOString() });
  emitEvent("integration", "StateTransition" as GameEventType, actorId, { integrationEventType: type, ...payload });
  log.info("bridge.publish", { type, actorId });
}

export function _resetBridgeForTesting(): void { unsubscribeIntegration(); processedEventIds.clear(); publishedEvents.length = 0; }
