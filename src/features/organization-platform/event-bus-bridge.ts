/** System 24 — Event Bus Bridge. Passive consumer + producer. */
import { getLogger } from "@/lib/logger";
import { subscribe, emitEvent, type GameEvent, type GameEventType } from "@/features/game-engine";
import type { OrganizationEventType } from "./types";

const log = getLogger("organization-platform.bridge");
const processedEventIds = new Set<string>();
const publishedEvents: Array<{ type: OrganizationEventType; actorId: string | null; payload: Record<string, unknown>; timestamp: string }> = [];
...
export function subscribeOrganizationPlatform(): void {
...
export function unsubscribeOrganizationPlatform(): void { for (const u of unsubscribers) { try { u(); } catch {} } unsubscribers.length = 0; isSubscribed = false; processedEventIds.clear(); }
export function isOrganizationPlatformSubscribed(): boolean { return isSubscribed; }
...
export function publishOrganizationEvent(type: OrganizationEventType, actorId: string | null, payload: Record<string, unknown>): void {
  publishedEvents.push({ type, actorId, payload, timestamp: new Date().toISOString() });
  emitEvent("organization-platform", "StateTransition" as GameEventType, actorId, { organizationEventType: type, ...payload });
  log.info("bridge.publish", { type, actorId });
}

export function _resetBridgeForTesting(): void { unsubscribeOrganizationPlatform(); processedEventIds.clear(); publishedEvents.length = 0; }
